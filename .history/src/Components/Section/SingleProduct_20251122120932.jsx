...
const SingleProduct = () => {
  ...
  //  const clickedProduct = data.find((item)=>{
  //   return item.id ===  Number(id)
  //  })
  ...
  return (
    <div className="bg-gray-50 py-12 px-6 lg:px-20">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        ...
        >
          <motion.img
            // src={`${clickedProduct.image}?auto=compress&cs=tinysrgb&h=400`}
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <h1 className="text-3xl font-bold text-gray-800">{clickedProduct.name}</h1>
          ...
          <div className="flex items-center gap-4 mt-6">
            <button
            onClick={()=>dispatch(decreaseQty(clickedProduct))}
              className="bg-gray-200 text-lg font-bold w-10 h-10 rounded-full hover:bg-gray-300"
            >
              –
            </button>
            <span className="text-xl font-medium">{clickedProduct.qty}</span>
            <button
            onClick={()=>dispatch(increaseQty(clickedProduct))}
              className="bg-gray-200 text-lg font-bold w-10 h-10 rounded-full hover:bg-gray-300"
            >
              +
            </button>
          ...
          {/* Add to Cart Button */}
          <motion.button
            onClick={()=>dispatch(addProduct(clickedProduct))}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          ...
          <p className="mt-6 text-gray-600 leading-relaxed">
            This {clickedProduct.name} is designed with high-quality materials and
